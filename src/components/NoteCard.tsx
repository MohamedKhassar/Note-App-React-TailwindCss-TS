...
const NoteCard = ({ backgroundColor, textColor, content, title, pinned, isFavorite, date, tags }: Note) => {
    return (
        <div className={`p-6 transition-all duration-300 rounded-xl cursor-pointer flex flex-col justify-between gap-4`} style={{ backgroundColor, color: textColor }}>
            ...
            <div className="flex text-nowrap justify-between text-xs items-center">
                <div className="flex gap-x-2 flex-wrap gap-3 items-center">
                    {tags.slice(0, 2).map((tag: string, key: number) => (
                    ...
                    {tags.length > 2 &&<span className="text-sm text-sky-200 bg-sky-900 rounded-full px-2.5 py-2 cursor-pointer hover:bg-sky-950 hover:text-sky-100 transition-all">#more...</span>}
